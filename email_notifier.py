...
def send_email(properties):
    ...
    try:
        ...
        for prop in properties:
            ...
            method = prop.get('mobile_store_search_method', 'Google Maps Web Search')
            radius = prop.get('mobile_store_search_radius', '100m')
            # Use HTML entity &check; instead of Unicode character
            mobile_store = f"No mobile stores detected within {radius} <span class='check'>&check;</span><br><small>Method: {method}</small>"
        ...
        html_content += """
            ...
            <p><strong>How Mobile Store Detection Works:</strong> We use direct Google Maps searches to find the exact 
            location of each Walmart, then search for nearby 
